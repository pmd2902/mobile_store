...
function openTab(nameTab) {
  ...
  switch (nameTab) {
    ...
    case "Đơn Hàng":
      ...
      break;
    case "Giỏ hàng":
      document.getElementsByClassName("giohang")[0].style.display = "block";
      break;
  }
}
...
function addTableGioHang() {
  var tc = document
    .getElementsByClassName("giohang")[0]
    .getElementsByClassName("table-content")[0];
  var s = `<table class="table-outline hideImg">`;

  var listGH = getListGioHang();

  TONGTIEN = 0;
  for (var i = 0; i < listGH?.length; i++) {
    var d = listGH[i];
    s +=
      `<tr>
            <td style="width: 4%">` +
...
function getListGioHang(traVeDanhSachSanPham = false) {
  var u = getListUser();
  var result = [];
  if (u && u.length > 0) {
    u.forEach((user) => {
      if (user.products && user.products.length > 0) {
        ...
        user.products.forEach((product) => {
          var timsp = timKiemTheoMa(list_products, product.ma);
          if (timsp) {
            var gia =
              timsp.promo.name === "giareonline"
                ? timsp.promo.value
                : timsp.price;
            tongtien += stringToNum(gia);

            sanPhamChuaXuLy.push({
              tenSanPham: timsp.name,
              soLuong: product.soluong,
            });
          }
        });

        var sps = sanPhamChuaXuLy
          .map(
            (sp) =>
              `<p style="text-align: right">${sp.tenSanPham} [${sp.soLuong}]</p>`
          )
          .join("");
        var x = new Date(
          user.products[user.products.length - 1]?.date
        ).toLocaleString();

        result.push({
          khach: username,
          sp: traVeDanhSachSanPham ? sanPhamChuaXuLy : sps,
    ...
    console.log(
      "Không có người dùng nào được tìm thấy hoặc không có sản phẩm trong giỏ hàng."
    );
  }
      ...
      if (u[i].donhang[j].ngaymua == maDonHang) {
        if (duyetDon) {
          if (u[i].donhang[j].tinhTrang == "Đang chờ xử lý") {
            u[i].donhang[j].tinhTrang = "Đang giao hàng";
